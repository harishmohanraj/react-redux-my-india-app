...
function filterListByDropDownSelection(list) {
	return list.filter((item) => item[0] !== "TOTAL (ALL-INDIA)" && item[0] !== "TOTAL (STATES)").slice(0,10)
}

export default function formatChartData(chartDataObj){
    let valueArray = [], nameArray = [];
    const sortDataList = chartDataObj.sort(sortList);
